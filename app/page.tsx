...
export default function Home() {
  ...
  const [gameMode, setGameMode] = useState<'x01' | 'cricket' | 'century' | 'scram'>('x01');
      ...
      <div className="w-full max-w-md bg-gray-800 p-6 sm:p-8 rounded-3xl shadow-2xl border border-gray-700/50 relative overflow-hidden">
        ...
        {step === 3 && (
          <div className="animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="flex flex-col gap-4 mb-8">
              {Array.from({ length: numPlayers }).map((_, i) => (
                <div key={i} className="flex flex-col gap-2 bg-gray-900/30 p-3 rounded-2xl border border-gray-700/50">
                  <input type="text" value={playerNames[i]} onChange={(e) => handleNameChange(i, e.target.value)} placeholder={`Joueur ${i + 1}`} maxLength={10} className={`w-full bg-gray-800 text-white px-4 py-3 rounded-xl border-2 transition-colors focus:outline-none ${gameMode === 'x01' ? 'focus:border-blue-500 border-gray-700' : gameMode === 'cricket' ? 'focus:border-green-500 border-gray-700' : gameMode === 'scram' ? 'focus:border-orange-500 border-gray-700' : 'focus:border-purple-500 border-gray-700'}`} />
            ...
            <button onClick={startGame} className={`w-full p-5 rounded-2xl text-2xl font-black transition-all active:scale-95 shadow-xl ${gameMode === 'x01' ? 'bg-blue-500 hover:bg-blue-400 shadow-blue-900/50' : gameMode === 'cricket' ? 'bg-green-500 hover:bg-green-400 shadow-green-900/50' : gameMode === 'scram' ? 'bg-orange-500 hover:bg-orange-400 shadow-orange-900/50' : 'bg-purple-500 hover:bg-purple-400 shadow-purple-900/50'}`}>
              JOUER !
            </button>
          </div>
        )}
